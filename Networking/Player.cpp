...
void Names::init(int player, int pnb, int my_port, const char* servername, bool setup_socket)
{
    player_no = player;
    portnum_base = pnb;
...
Names::Names(ez::ezOptionParser& opt, int argc, const char** argv, int default_nplayers) : Names()
{
...
#ifdef PPC_COMMUNICATION
void PlainPlayer::setup_sockets(
    const vector<string>& names, const vector<int>& ports, const string& id_base)
#else
void PlainPlayer::setup_sockets(const vector<string>& names, const vector<int>& ports,
...
#endif
{
    ...
    for (int i = 0; i < nplayers; i++)
    {
        ...
        tv.tv_usec = 0;
        int fl =
            setsockopt(sockets[i], SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, sizeof(struct timeval));
        if (fl < 0)
        {
...
size_t PlainPlayer::send_no_stats(int player, const PlayerBuffer& buffer, bool block) const
{
    if (block)
    {
...
size_t PlainPlayer::recv_no_stats(int player, const PlayerBuffer& buffer, bool block) const
{
    if (block)
    {
...
template <class T>
void MultiPlayer<T>::exchange_no_stats(
    int other, const octetStream& o, octetStream& to_receive) const
{
...
template <class T>
void MultiPlayer<T>::pass_around_no_stats(
    const octetStream& o, octetStream& to_receive, int offset) const
{
...
void MultiPlayer<T>::Broadcast_Receive_no_stats(vector<octetStream>& o) const
{
    ...
    for (int i = 1; i < nplayers; i++)
    {
        ...
        exchangers.push_back(
            {sockets[send_to], o[my_num()], sockets[receive_from], o[receive_from]});
    }
...
void Player::send_receive_all(
    const vector<octetStream>& to_send, vector<octetStream>& to_receive) const
{
    send_receive_all(vector<vector<bool>>(num_players(), vector<bool>(num_players(), true)),
        to_send, to_receive);
}

void Player::send_receive_all(const vector<bool>& senders, const vector<octetStream>& to_send,
...
void Player::partial_broadcast(
    const vector<bool>&, const vector<bool>&, vector<octetStream>& os) const
{
    unchecked_broadcast(os);
}

template <class T>
void MultiPlayer<T>::send_receive_all_no_stats(const vector<vector<bool>>& channels,
    const vector<octetStream>& to_send, vector<octetStream>& to_receive) const
{
...
ThreadPlayer::~ThreadPlayer()
{
    for (unsigned int i = 0; i < receivers.size(); i++)
    {
        if (receivers[i]->timer.elapsed() > 0)
            cerr << "Waiting for receiving from " << i << ": " << receivers[i]->timer.elapsed()
                 << endl;
        delete receivers[i];
    }
...
RealTwoPartyPlayer::RealTwoPartyPlayer(const Names& Nms, int other_player, int id_base)
  : RealTwoPartyPlayer(Nms, other_player, to_string(id_base))
{}
...
VirtualTwoPartyPlayer::VirtualTwoPartyPlayer(Player& P, int other_player)
  : TwoPartyPlayer(P.my_num()),
    P(P),
    other_player(other_player),
    comm_stats(P.thread_stats.at(other_player))
{}
...
void NamedCommStats::print(bool newline)
{
    for (auto it = begin(); it != end(); it++)
        if (it->second.data)
            cerr << it->first << " " << 1e-6 * it->second.data << " MB in " << it->second.rounds
                 << " rounds, taking " << it->second.timer.elapsed() << " seconds" << endl;
    if (size() and newline)
        cerr << endl;
}
